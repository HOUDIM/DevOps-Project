...
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <c:if test="${sousCategorie.id!=null}">
    <title>Modifier Sous Categorie</title>
  </c:if>
  <c:if test="${sousCategorie.id==null}">
          ...
          <div class="page-title">
            <div class="row">
              <div class="col-12 col-md-6 order-md-1 order-last">
                <c:if test="${sousCategorie.id!=null}">
                  <h3>Modification de ${sousCategorie.nom}</h3>
                </c:if>
                <c:if test="${sousCategorie.id==null}">
                  <h3>Ajout d'une sous categorie</h3>
                </c:if>
              ...
              <div class="col-12">
                <div class="card">
                  ...
                  <div class="card-content">
                    <div class="card-body">
                      <form class="form" action="/app/sousCategorie/sauvgarder" method="post" enctype="multipart/form-data">
                        <div class="row">
                          <div class="col-md-6 col-12">
                            <div class="form-group">
                              <label class="form-label" for="basicInput">Nom</label>
                                <input
                                  type="text"
                                  class="form-control"
                                  id="basicInput"
                            ...
                            <label for="formFileMultiple" class="form-label"
                              ...
                              <select class="form-select" id="basicSelect" name="categorie">
                                ...
                                <c:forEach items="${categorie}" var="cc" >
                                  <option value=${cc.id} <c:if test="${sousCategorie.categorie.id==cc.id}">selected="true"</c:if>>${cc.nom} </option>
                                </c:forEach>
                              </select>
                          </div>
                         </div>
